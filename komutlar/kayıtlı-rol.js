...
if(!message.member.hasPermission("ADMINISTRATOR")) return message.channel.send(new Discord.MessageEmbed().setDescription(`${ayarlar.hata2} | Bu komutu kullanabilmek için gerekli yetkiye sahip değilsin!`).setColor(hata))
  ...
  Selam, Kayıtlı Rolünü Ayarlamak İçin Aşşağıda Verdiğimiz Seçenekleri Kullan!:
  
  \`${prefix}k-kayıtlı-rol @rol\`
  \`${prefix}k-kayıtlı-rol kapat\`
  ...
  message.channel.send(new Discord.MessageEmbed().setDescription(`${ayarlar.oldu2} | Kayıtlı rolü başarıyla kapatıldı`).setColor(oldu))
    return
  }
  ...
  message.channel.send(new Discord.MessageEmbed().setDescription(`${ayarlar.oldu2} | Kayıtlı rolü başarıyla ${type} olarak ayarlandı!`).setColor(oldu))
  return
...
module.exports.conf = {
  enabled: true,
  guildOnly: false,
  aliases: ["kayıtlı-rol", "kayıtlırol"],
  permLevel: 0
};
